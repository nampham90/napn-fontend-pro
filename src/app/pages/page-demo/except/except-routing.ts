...
export default [
  ...
  { path: 'network-error', data: { title: 'Lỗi mạng', key: 'network-error' }, loadComponent: () => import('./network-error/network-error.component').then(m => m.NetworkErrorComponent) },
  { path: 'no-data', data: { title: 'không có dữ liệu', key: 'no-data' }, loadComponent: () => import('./no-data/no-data.component').then(m => m.NoDataComponent) }
] as Route[];
